...
for i in range(len(query_list)):
    ...
    bm25_text_list=[text_list[g] for g in top_k_indices_recall_bge]
    bm25=BM25Okapi(bm25_text_list)
    bm25_scores = bm25.get_scores(tokenized_query)
    top_k_indices_recall_bm25 = sorted(range(len(bm25_scores)), key=lambda i: bm25_scores[i], reverse=True)[:top_k_bm25]

    rerank_list_bm25=[]
    for g in top_k_indices_recall_bm25:
        rerank_list_bm25.append([query_list[i],bm25_text_list[g]])
    ...
    top_k_indices_real =[top_k_indices_recall_bge[ele] for ele in top_k_indices_bm25]
    ...
    print("Bm25-Top-{} 最相关的文档url: {}".format(top_k_final, [url_list[i] for i in top_k_indices_real]))
    for j in top_k_indices_real:
